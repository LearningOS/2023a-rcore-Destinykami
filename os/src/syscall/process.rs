...
use crate::{
    ...
    task::{
        change_program_brk, exit_current_and_run_next, suspend_current_and_run_next, TaskStatus, current_user_token,get_current_task_info,mmap,munmap,
    },
    timer::get_time_us,
...
pub fn sys_task_info(_ti: *mut TaskInfo) -> isize {
    ...
    let p_ti = translated_mut_ptr(token, _ti);
    ...
    (*p_ti).status=status;
    (*p_ti).time=time;
    (*p_ti).syscall_times=syscall_times;
    0
}
